...
*----------------------------------------------------------------------
*  Title:
*    Objects 301: Design Patterns: Singleton pattern, step 2
...
*   In the previous version we introduced the Singleton
*   design pattern, having defined a new global variable
*   to hold a reference to the new "report" instance along
*   with new static method "get_singleton_instance" of class
*   "report" so we could retrieve a reference to this instance.
*
*   You may recall that during the earliest exercise programs
*   we made quite an effort to eliminate all global variables
*   from the program; yet with the previous version we found
*   ourselves defining a new one.  This version demonstrates
*   how we can eliminate this global variable as well.
*
*   Class "report" is changed to move the static singleton
*   attribute from the private section to the public section,
*   and now also is assigned the "read-only" modifier, which
*   grants external entities "read" access to this public
*   attribute but not "write" access.  Now having provided
*   read access to this attribute, there no longer is a need
*   for static method "get_singleton_instance", so it is deleted.
*
*   Now that the ABAP classic event blocks have access to the
*   static singleton attribute of the "report" class, there no
*   longer is a reason to retain the global variable previously used,
*   so it is deleted, along with the call by the ABAP classic event
*   block "initialization" to the now-defunct method
*   "get_singleton_instance" of the "report" class.  Finally, those
*   statements found in the ABAP classic event blocks previously
*   invoking the methods of the "report" instance via the global
*   variable are changed to reference those methods instead via the
*   static singleton attribute of the "report" class, changing
*   statements beginning with:
*       "call method report->..."
*   to:
*       "call method report=>singleton->..."
*
