...
*----------------------------------------------------------------------
*  Title:
*    Objects 101: Car report using one local static class
...
*   A local class is defined for "car", which encapsulates class attributes
*   for car-relevant constants, types and data fields previously defined as
*   global variables, as well as car-relevant methods previously defined
*   as classical subroutines.  Since the program can accommodate only one
*   car entry, the attributes and methods of the "car" class are defined as
*   static (class-methods and class-data), eliminating the need to create
*   an instance of a "car" class.  Visibility for a class component is marked
*   as restrictive as possible and only as visible as necessary.
*
...
class car definition abstract final create private.
  public section.
    types        : brand_type     type f4txt
                 ...
                 .
    constants    : left_turn      type car=>turn_type
                                                 value 'L'
                 , right_turn     type car=>turn_type
                                                 value 'R'
                 , u_turn         type car=>turn_type
                                                 value 'U'
                 .
...
*   Any turn value other than a valid navigator turn will be ignored:
    check turn eq car=>left_turn
       or turn eq car=>right_turn
       or turn eq car=>u_turn.
*   Convert current heading to string offset:
    find car=>heading in car=>compass match offset compass_offset.
...
*   Adjust numeric value to accommodate underflow or overflow:
    if compass_offset lt car=>compass_offset_limit_lo.
      add      04 to   compass_offset.
    endif.
    if compass_offset gt car=>compass_offset_limit_hi.
      subtract 04 from compass_offset.
    endif.
*   Reset heading:
    car=>heading                  = car=>compass+compass_offset(01).
  endmethod.
*----------------------------------------------------------------------
...
*   default to the first navigator compass heading:
    if car=>compass ca heading.
      car=>heading                = heading.
    else.
      car=>heading                = car=>compass+00(01).
    endif.
  endmethod.
...
parameters       :   pplate       type car=>license_plate_type
                 ...
                 ,   pheading     type car=>heading_type
                 ,   pturn01      type car=>turn_type
                 ,   pturn02      type car=>turn_type
                 ,   pturn03      type car=>turn_type
