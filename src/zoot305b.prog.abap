...
*----------------------------------------------------------------------
*  Title:
*    Objects 305: Design Patterns: Adapter pattern, step 2
...
*   This version continues with the Adapter design pattern.  In the
*   previous version we already had accommodated the announcement
*   by the makers of the commercial gps about dropping support of the
*   "simple_navigation" interface, and we used inheritance to create
*   class "commercial_gps_adapter_a" as a subclass to class
*   "commercial_gps".  In this version we are going to use class
*   composition instead of class inheritance.
*
*   We will keep class "commercial_gps_adapter_a" but will no longer use
*   it.  Instead, we will create a new class "commercial_gps_adapter_b"
*   and use it as a substitute for "commercial_gps_adapter_a".
*
*   New class "commercial_gps_adapter_b" starts as a copy of class
*   "commercial_gps_adapter_a" and differs in the following ways:
*     o it does not inherit from class "commercial_gps"
*     o the value of its class_id is changed accordingly
*     o it defines a private attribute which is a reference to an instance
*       of class commercial_gps
*     o its constructor method creates an instance of a commercial_gps
*       into the corresponding new private attribute
*     o its method "get_heading" invokes the "get_bearing" method of
*       its private attribute instance of commercial_gps
*     o its method "change_heading" invokes the "change_bearing" method of
*       its private attribute instance of commercial_gps
*   Because this new class encapsulates an instance of class
*   "commercial_gps" as a private attribute, it means that an instance
*   of one class -- "commercial_gps" -- is used to compose the content of
*   another class --  "commercial_gps_adapter_b".  Stated another way,
*   the "composition" of class "commercial_gps_adapter_b" includes an
*   instance of class "commercial_gps".  This is what is meant by
*   "class composition".  It also means the instance of class
*   "commercial_gps" can be accessed only by going through the instance
*   of class "commercial_gps_adapter_b".
*
*   Class "commercial_gps" continues to perform all of its processing
*   solely on the basis of bearings.  Class "commercial_gps_unit_maker"
*   is changed so that it now creates an instance of class
*   "commercial_gps_adapter_b" instead of class "commercial_gps_adapter_a".
*
*   Also, in the previous version we changed the definition statement for
*   class "commercial_gps" to show it is no longer final.  This is only
*   required now because we still have the unused "commercial_gps_adapter_a"
*   class inheriting from it.  If we were to remove the definition and
*   implementation of class "commercial_gps_adapter_a" altogether, then
*   there would no longer be a reason for "commercial_gps" to remain
*   defined as an inheritable class, and we could reinstate the "final"
...
class commercial_gps definition.
  public section.
    types        : bearing_type   type int4
                 .
    methods      : constructor
                     importing
                       bearing
                         type commercial_gps=>bearing_type
                 , change_bearing
                     importing
                       change
                         type commercial_gps=>bearing_type
...
* protected section.
  private section.
    constants    : degrees_360    type int4      value 360
                 .
    data         : bearing        type commercial_gps=>bearing_type
                 .
endclass. " commercial_gps definition
*----------------------------------------------------------------------
*
* Class: commercial_gps_adapter_a definition
*
*----------------------------------------------------------------------
class commercial_gps_adapter_a definition final inheriting from commercial_gps.
  public section.
    interfaces   : simple_navigation
                 .
    aliases      : change_heading
                 ...
                 , get_heading
                     for simple_navigation~get_heading
                 .
    constants    : class_id       type seoclsname value 'COMMERCIAL_GPS_ADAPTER_A' "#EC *
                 .
    methods      : constructor
                     importing
                       heading
...
* protected section.
  private section.
    constants    : degrees_90     type int4      value 90
                 , degrees_180    type int4      value 180
                 .
endclass. " commercial_gps_adapter_a definition
*----------------------------------------------------------------------
*
* Class: commercial_gps_adapter_b definition
*
*----------------------------------------------------------------------
class commercial_gps_adapter_b definition final.
  public section.
    interfaces   : simple_navigation
                 .
    aliases      : change_heading
                 ...
                 , get_heading
                     for simple_navigation~get_heading
                 .
    constants    : class_id       type seoclsname value 'COMMERCIAL_GPS_ADAPTER_B'
                 .
    methods      : constructor
                     importing
                       heading
...
* protected section.
  private section.
    constants    : degrees_90     type int4      value 90
                 , degrees_180    type int4      value 180
                 .
    data         : adapted_gps    type ref
                                    to commercial_gps
                 .
endclass. " commercial_gps_adapter_b definition
*----------------------------------------------------------------------
*
...
*----------------------------------------------------------------------
*
* Class: commercial_gps_adapter_a implementation
*
*----------------------------------------------------------------------
class commercial_gps_adapter_a implementation.
*----------------------------------------------------------------------
...
*----------------------------------------------------------------------
  method change_heading.
    data         : bearing_change type commercial_gps=>bearing_type
                 .
*   Any turn value other than a valid navigator turn will be ignored:
    check turn eq simple_navigation=>left_turn
       or turn eq simple_navigation=>right_turn
...
*   by 180:
    case turn.
      when simple_navigation=>left_turn.
        subtract commercial_gps_adapter_a=>degrees_90
                                  from bearing_change.
      when simple_navigation=>right_turn.
        add      commercial_gps_adapter_a=>degrees_90
                                  to   bearing_change.
      when simple_navigation=>u_turn.
        add      commercial_gps_adapter_a=>degrees_180
                                  to   bearing_change.
    endcase.
*   Apply bearing change via inherited method:
    call method me->change_bearing
      exporting
...
*----------------------------------------------------------------------
  method get_heading.
    data         : compass_offset type int4
                 , bearing        type commercial_gps=>bearing_type
                 .
*   Get current bearing via inherited method:
    call method me->get_bearing
      importing
        bearing                   = bearing
        .
    compass_offset                = bearing / commercial_gps_adapter_a=>degrees_90.
    heading                       = simple_navigation=>compass+compass_offset(01).
  endmethod.
*----------------------------------------------------------------------
* Method: constructor
...
*   default to north (00 degrees):
    find heading in simple_navigation=>compass match offset compass_offset.
    bearing                       = compass_offset * commercial_gps_adapter_a=>degrees_90.
*   Call super constructor:
    call method super->constructor
      exporting
        ...
        .
  endmethod.
endclass. " commercial_gps_adapter_a implementation
*----------------------------------------------------------------------
*
* Class: commercial_gps_adapter_b implementation
*
*----------------------------------------------------------------------
class commercial_gps_adapter_b implementation.
*----------------------------------------------------------------------
...
*   by 180:
    case turn.
      when simple_navigation=>left_turn.
        subtract commercial_gps_adapter_b=>degrees_90
                                  from bearing_change.
      when simple_navigation=>right_turn.
        add      commercial_gps_adapter_b=>degrees_90
                                  to   bearing_change.
      when simple_navigation=>u_turn.
        add      commercial_gps_adapter_b=>degrees_180
                                  to   bearing_change.
    endcase.
*   Delegate bearing change to adapted gps unit:
    call method me->adapted_gps->change_bearing
      exporting
        change                    = bearing_change
        .
  endmethod.
...
*----------------------------------------------------------------------
  method get_heading.
    data         : compass_offset type int4
                 , bearing        type commercial_gps=>bearing_type
                 .
*   Get current bearing from adapted gps unit:
    call method me->adapted_gps->get_bearing
      importing
        bearing                   = bearing
        .
    compass_offset                = bearing / commercial_gps_adapter_b=>degrees_90.
    heading                       = simple_navigation=>compass+compass_offset(01).
  endmethod.
*----------------------------------------------------------------------
* Method: constructor
...
*   default to north (00 degrees):
    find heading in simple_navigation=>compass match offset compass_offset.
    bearing                       = compass_offset * commercial_gps_adapter_b=>degrees_90.
*   Create instance of adapted gps unit:
    create object me->adapted_gps
      exporting
        bearing                   = bearing
        .
  endmethod.
endclass. " commercial_gps_adapter_b implementation
*----------------------------------------------------------------------
*
...
*----------------------------------------------------------------------
  method create_unit.
    unit_type                     = commercial_gps_adapter_b=>class_id.
    create object navigation_unit type (unit_type)
      exporting
        heading                   = heading
        .
