...
*----------------------------------------------------------------------
*  Title:
*    Objects 305: Design Patterns: Adapter pattern, step 1
...
*   This version illustrates the Adapter design pattern.  The
*   makers of the commercial gps have notified their customers that
*   they no longer will support the "simple_navigation" interface,
*   and now will require users of the unit to specify their
*   requests using a bearing number instead of a compass heading.
*
*   We still want to use it in our trucks, but we do not want to
*   change the way trucks interact with the unit.  A simple
*   solution is to place an adapter between class "commercial_gps"
*   and its callers.  The adapter class will convert calls from
*   the simple_navigation format into a format accepted by class
*   "commercial_gps".
*
*   New navigation unit class "commercial_gps_adapter_a" is defined
*   to take the place of class "commercial_gps", accepting requests
*   through the "simple_navigation" interface and passing these requests
*   on to "commercial_gps" using only bearing numbers.  Indeed, the
*   existing definition and implementation of "commercial_gps" can be
*   renamed as "commercial_gps_adapter_a" to get started.  Its definition
*   indicates it inherits from class "commercial_gps", and constitutes
*   an example of defining an adapter using inheritance.  The methods
*   constructor, "change_heading" and "get_heading" originally defined for
*   "commercial_gps" are retained for class "commercial_gps_adapter_a"
*   but changed accordingly to facilitate calls to the super constructor,
*   "change_bearing" and "get_bearing", respectively, of the super class.
*
*   Class "commercial_gps" is changed so that it enables subclassing --
*   specifically, the "final" keyword has been removed from the class
*   definition statement.  Its interfaces statement and aliases facilitating
*   the interface "simple_navigation" have been removed.  It now defines
*   a public type "bearing", defined as type integer, and a private attribute
*   to hold a bearing.  Its constructor method is changed to accept a bearing
*   instead of a heading.  It also now has a new method "get_bearing", which
*   sends the private bearing value back to the caller, and a new method
*   "change_bearing", which accepts a bearing value and adds this to the
*   private bearing attribute, adjusting as necessary so the updated bearing
*   value remains between zero and 360 degrees.  This class now performs all
*   of its processing solely on the basis of bearings.
*
*   Class "commercial_gps_unit_maker" is changed so that its "create_unit"
*   method now will create an instance of class "commercial_gps_adapter_a"
*   instead of class "commercial_gps".
*
  ...
  public section.
    ...
    aliases      : change_heading
                 ...
                 , get_heading
                     for simple_navigation~get_heading
                 .
    constants    : class_id       type seoclsname value 'COMMERCIAL_GPS_ADAPTER_A'
                 .
    methods      : constructor
                     importing
                       heading
...
*----------------------------------------------------------------------
  method create_unit.
    unit_type                     = commercial_gps_adapter_a=>class_id.
    create object navigation_unit type (unit_type)
      exporting
        heading                   = heading
        .
